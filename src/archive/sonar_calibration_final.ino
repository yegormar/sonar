...
float measureOnce(int trigPin, int echoPin) {
  ...
  // Smart reflection filtering - allow up to 2m but prefer closer readings
  // During calibration, we expect consistent distances
  if (distance > 200.0) return -1.0; // Hard cutoff at 2m
  
  return distance;
}

float avgDistance(int trigPin, int echoPin) {
  // Use array to collect all readings for advanced filtering
  float readings[40]; 
  int count = 0;
  ...
  // If we have very few readings, take more
  if (count < 5) {
    Serial.println(F("  Note: Few valid readings. Taking additional samples..."));
    for (int i = 0; i < 20; i++) {
      float d = measureOnce(trigPin, echoPin);
      if (d > 0 && count < 40) {
        readings[count++] = d;
  ...
  // Advanced filtering for your specific setup with wall at ~2m
  // Look for consistent readings that appear multiple times
  ...
  // Find the most common reading (mode) - this helps reject wall reflections
  float bestReading = readings[count/2]; // Start with median
  int bestCount = 1;
  ...
  for (int i = 1; i < count; i++) {
    if (abs(readings[i] - current) < 5.0) { // Within 5cm
      currentCount++;
    } else {
  ...
  // If we found a consistent reading, use it
  if (bestCount >= 3) {
    Serial.print(F("  Using consistent reading: "));
    Serial.print(bestReading, 1);
    Serial.println(F(" cm (appeared "));
    Serial.print(bestCount);
    Serial.println(F(" times)"));
    return bestReading;
  }
  
  // Fall back to median if no consistent reading found
  Serial.println(F("  No consistent reading found - using median"));
